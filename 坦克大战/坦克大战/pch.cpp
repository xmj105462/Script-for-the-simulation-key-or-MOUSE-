		...
		BULLET_UP//子弹打到不可消墙
			...
			for (size_t i = 0; i < eARR_size; i++) {
				...
				if (enemyArr[i].x == (biu->x) && enemyArr[i].y == (biu->y-1)) {
					...
					enemyArr[i].live = false; ENEMY_INIT DELETE_B break;
				}
					...
					for (size_t i = 0; i < BLAST * 2; i++) {
						...
						enemyArr[i].live = false; ENEMY_INIT2 DELETE_B break;
					}
...
void play() {
	...
	while (1) {
		if (!enemyArr.empty())bulletMove(&bullImg, blastImg, &enemyArr[0], enemyArr.size());
		else break;
		if (!enemyArr.empty()) {
		...
		if (!enemyArr.empty()) {
			for (size_t i = 0; i < enemyArr.size(); i++) {
				...
				//子弹行动				
				if (enemyArr[i].enemyBulletMove(blastImg, &eBullImg, &myTank))return;
			}
		}t2 = _GetSysTickCount64();
	}endGame(false);
}
...
bool Enemy::enemyBulletMove(IMAGE* blastImg, IMAGE* eBullImg, Tank* myTank) {
	//子弹live活的才动
	if (bullet == nullptr) return false;
	if (bullet->live==true) {			
		if (bullet->bDir == UP) {
			...
			eBULLET_UP//子弹打到不可消墙
			else if (map[bullet->y][bullet->x] == 100 || map[bullet->y][bullet->x] == 200) {
			...
			else if (map[bullet->y][bullet->x] == 100 || map[bullet->y][bullet->x] == 4 || map[bullet->y][bullet->x] == 200) {//待加线程
			...
			else if ((map[bullet->y][bullet->x] == 100 || map[bullet->y][bullet->x] == 4 || map[bullet->y][bullet->x] == 200) && bullet->x > 0) {//待加线程		
			...
			else if ((map[bullet->y][bullet->x] == 100 || map[bullet->y][bullet->x] == 4||map[bullet->y][bullet->x] == 200) && bullet->x < (MAP_Y - 1)) {//待加线程
					if (myTank->x == bullet->x && myTank->y == bullet->y) {
						clearrectangle((bullet->x) * IMAGE_SIZE, (bullet->y) * IMAGE_SIZE, (bullet->x + 2) * IMAGE_SIZE, (bullet->y + 2) * IMAGE_SIZE);
						mciSendString(_T("play res/hit.mp3"), 0, 0, 0);//		
						for (size_t i = 0; i < BLAST * 2; i++) {
